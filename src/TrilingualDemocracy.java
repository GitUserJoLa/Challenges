...
"""
Switzerland has four official languages: German [D], French [F], Italian [I], and Romansh [R].
...
public class TrilingualDemocracy {
    public String chooseLanguage(String speakers) {
        ...
        //if(a=b and b=c => all letters are equal) return a
        if (letters[0].equals(letters[1]) &&
                letters[2].equals(letters[1]))
            return letters[0];

        //if(a!=b and a!=c and b!=c => all letters are different) return letter d which isn't in the array
        if (!(letters[0].equals(letters[1])) &&
            ...
            languageList.removeAll(List.of(letters));

            //return languageList.toString(); //has cosmetic output issues
            //convert list into array to resolve cosmetic output issues
    ...
    public static void main(String[] args) {
        ...
        System.out.println("IDF: " + tD.chooseLanguage("IDF"));
    }
}
